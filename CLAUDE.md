...
- **El monitoreo de posiciones abiertas corre a la misma velocidad que el loop principal (5 min)**, no en tiempo real — aunque el backend C# sí tickea mark price cada 1s y ES el que ejecuta el TP/SL fijo (sin este lag). El TP/SL se define al abrir y se ejecuta tal cual — no hay trailing/Cosecha Inteligente moviendo el punto de salida (se eliminó explícitamente el 2026-07-15, ver progress log; si el TP/SL de entrada no está bien calibrado, ese es el problema a resolver ahí, no compensarlo con monitoreo).
