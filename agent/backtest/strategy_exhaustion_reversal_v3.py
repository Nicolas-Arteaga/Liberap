...
PRIOR_MOVE_PCT = 1.0   # calibrado para alts (v2 usaba 0.5%, para BTC) -- alt_breakout_mining.py
                        # confirmo el patron con umbral de ruptura 3% en alts vs 1.5% en BTC (~2x), mismo factor aca
...
def main():
    ...
    print(f"Reversion por Agotamiento v3 (calibrada para alts) | RSI(14) {RSI_LOW}/{RSI_HIGH} + confirmacion de giro | "
          f"movimiento previo >={PRIOR_MOVE_PCT}% | TP={ATR_TP_MULT}xATR SL={ATR_SL_MULT}xATR (R:R {ATR_TP_MULT/ATR_SL_MULT:.1f}:1)", flush=True)
    print(f"Canasta: {len(BASKET)} simbolos (patron validado con lift real cross-alt en alt_breakout_mining.py)", flush=True)
