...
def backtest(conn, basket, tf_min, atr_sl_mult, rr_mult, ma_pair, side_filter, cache):
    if tf_min not in cache:
        cache[tf_min] = {}
    all_trades = []
    strat = {"entry_type": "ma_cross", "ma_pair": ma_pair}
    for symbol in basket:
        if symbol not in cache[tf_min]:
            ...
            cache[tf_min][symbol] = (candles, closes, highs, lows, atr)
        cached = cache[tf_min][symbol]
        if cached is None:
            continue
        candles, closes, highs, lows, atr = cached
        open_trade = None
...
def main():
    ...
    cache = {}
    total_combos = len(TIMEFRAMES) * len(ATR_SL_MULTS) * len(RR_MULTS) * len(MA_PAIRS) * len(SIDES)
        ...
        for sl_mult in ATR_SL_MULTS:
            for rr in RR_MULTS:
                for ma_pair in MA_PAIRS:
                    for side in SIDES:
                        tried += 1
                        trades = backtest(conn, basket, tf, sl_mult, rr, ma_pair, side, cache)
                        n = len(trades)
                        if n < 20:
                            continue
                        ...
                        results.append({"tf": tf, "sl_mult": sl_mult, "rr": rr, "ma": ma_pair, "side": side,
    ...
    print(f"\n=== TOP 15 ma_cross por $/mes (vetos+3cupos reales) ===")
    for r in results[:15]:
        print(f"  {r['side']:6s} tf={r['tf']:3d}m SL={r['sl_mult']}x RR={r['rr']}x MA={r['ma']} | n={r['n']:4d} WR={r['wr']:5.1f}% -> ${r['monthly']:8.2f}/mes")
