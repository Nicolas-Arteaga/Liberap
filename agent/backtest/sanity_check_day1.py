...
logging.basicConfig(level=logging.ERROR, handlers=[logging.FileHandler(os.path.join(os.path.dirname(__file__), "sanity_day1.log"), encoding="utf-8")])
...
def main():
    ...
    result = engine.run_parallel("FVG", PROFILE, symbols, START_MS, END_MS, max_workers=2)
    ...
    for t in sorted(result["trades"], key=lambda x: x["open_time"]):
        print(f"  {t['symbol']:15s} side={t['side']} open={datetime.fromtimestamp(t['open_time']/1000,tz=timezone.utc)} entry={t.get('entry_price')} pnl=${t['pnl']:.2f}")
