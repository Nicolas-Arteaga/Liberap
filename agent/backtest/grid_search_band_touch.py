...
from backtest.strategy_lab import load_basket, atr_series, resample, load_15m, DB_PATH, MARGIN, FEE, sma
...
def build_candidate(symbol, side, entry):
    return {
        ...
        "source": "band_touch_mode:grid", "band_touch_mode": True,
...
def backtest(conn, basket, tf_min, atr_sl_mult, rr_mult, side_filter, cache):
    ...
    for symbol in basket:
        ...
        for i in range(210, len(candles)):
            ...
            sig = None
            if c > upper and side_filter != "LONG":
                sig = 1
            elif c < lower and side_filter != "SHORT":
                sig = 0
            if sig is None:
                continue
            entry = c
...
def main():
    ...
    cache = {}
    total_combos = len(TIMEFRAMES) * len(ATR_SL_MULTS) * len(RR_MULTS) * len(SIDES)
    ...
    for tf in TIMEFRAMES:
        for sl_mult in ATR_SL_MULTS:
            for rr in RR_MULTS:
                for side in SIDES:
                    tried += 1
                    trades = backtest(conn, basket, tf, sl_mult, rr, side, cache)
                    n = len(trades)
                    if n < 20:
                        continue
                    ...
                    results.append({"tf": tf, "sl_mult": sl_mult, "rr": rr, "side": side,
    ...
    print(f"\n=== TOP 15 band_touch por $/mes (vetos+3cupos reales) ===")
    for r in results[:15]:
        print(f"  {r['side']:6s} tf={r['tf']:3d}m SL={r['sl_mult']}x RR={r['rr']}x | n={r['n']:4d} WR={r['wr']:5.1f}% -> ${r['monthly']:8.2f}/mes")
