...
from backtest.strategy_lab import load_basket, atr_series, resample, load_15m, DB_PATH, MARGIN, FEE, rsi_at
...
def backtest(conn, basket, tf_min, atr_sl_mult, rr_mult, hi_th, lo_th, side_filter, cache):
    ...
    for symbol in basket:
        if symbol not in cache[tf_min]:
            ...
            cache[tf_min][symbol] = (candles, closes, atr)
        cached = cache[tf_min][symbol]
        if cached is None:
            continue
        candles, closes, atr = cached
        open_trade = None
        for i in range(210, len(candles)):
            ...
            if r_now is None or r_prev is None:
                continue
            sig = None
            if r_prev >= hi_th and r_now < hi_th and side_filter != "LONG":
                sig = 1
            elif r_prev <= lo_th and r_now > lo_th and side_filter != "SHORT":
                sig = 0
            if sig is None:
                continue
            entry = c
...
def main():
    ...
    cache = {}
    total_combos = len(TIMEFRAMES) * len(ATR_SL_MULTS) * len(RR_MULTS) * len(RSI_THRESH) * len(SIDES)
    ...
    for tf in TIMEFRAMES:
        for sl_mult in ATR_SL_MULTS:
            for rr in RR_MULTS:
                for hi_th, lo_th in RSI_THRESH:
                    for side in SIDES:
                        tried += 1
                        trades = backtest(conn, basket, tf, sl_mult, rr, hi_th, lo_th, side, cache)
                        n = len(trades)
                        if n < 20:
                            continue
                        ...
                        results.append({"tf": tf, "sl_mult": sl_mult, "rr": rr, "rsi": f"{hi_th}/{lo_th}", "side": side,
    ...
    print(f"\n=== TOP 15 rsi_extreme por $/mes (vetos+3cupos reales) ===")
    for r in results[:15]:
        print(f"  {r['side']:6s} tf={r['tf']:3d}m SL={r['sl_mult']}x RR={r['rr']}x RSI={r['rsi']} | n={r['n']:4d} WR={r['wr']:5.1f}% -> ${r['monthly']:8.2f}/mes")
