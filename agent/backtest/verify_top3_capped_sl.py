"""
2026-08-18: mismo motor honesto (vetos reales + 3 cupos x $150) que
verify_top3_lab_strategies.py, pero con tope de SL fijo -$5 (nunca se
toca el TP, mismo patron que _capped_sl_tp_dist en verge_agent.py) para
Level Sweep 15m SL3/RR3 y 60m SL1.5/RR3 -- pedido directo del usuario.
"""
...
def backtest_level_sweep(conn, basket, tf_min, atr_sl_mult, rr_mult, lookback, cap_sl):
    all_trades = []
    for symbol in basket:
        ...
        for i in range(210, len(candles)):
            ...
            tp_dist = sl_dist_orig * rr_mult  # TP SIEMPRE del SL original, nunca se toca
            qty = MARGIN / entry
            if cap_sl:
            ...
            if not v_ok:
                continue
            open_trade = {"side": sig, "entry": entry, "sl": sl, "tp": tp, "open_time": ts, "qty": qty}
...
def report(label, trades, days=DAYS):
    ...
    win_sum = sum(wins); loss_sum = sum(losses)
    print(f"  {label:20s} n={n:5d} WR={len(wins)/n*100:5.1f}% neto=${total:9.2f} (${monthly:7.2f}/mes) | bruto_gana=${win_sum/(days/30.44):7.2f}/mes bruto_pierde=${loss_sum/(days/30.44):8.2f}/mes | peor_perdida=${min(losses) if losses else 0:.2f}")
...
def main():
    ...
    t0 = time.time()
    r = backtest_level_sweep(conn, basket, 15, 3.0, 3.0, 20, cap_sl=False)
    report("SIN TOPE", r)
    r = backtest_level_sweep(conn, basket, 15, 3.0, 3.0, 20, cap_sl=True)
    report("TOPE -$5", r)
    print(f"  ({time.time()-t0:.1f}s)\n")

    print("=== Level Sweep 60m SL1.5/RR3 ===")
    t0 = time.time()
    r = backtest_level_sweep(conn, basket, 60, 1.5, 3.0, 10, cap_sl=False)
    report("SIN TOPE", r)
    r = backtest_level_sweep(conn, basket, 60, 1.5, 3.0, 10, cap_sl=True)
    report("TOPE -$5", r)
    print(f"  ({time.time()-t0:.1f}s)")
