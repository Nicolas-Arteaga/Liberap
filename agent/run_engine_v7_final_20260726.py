...
out = {k:v for k,v in result.items() if k not in ('trades','all_signals_raw','shadow_signals')}
print(json.dumps(out, indent=2, ensure_ascii=False), flush=True)
with open('ma_slope_case3_ENGINE_v7_FINAL_20260726.json', 'w', encoding='utf-8') as f:
    json.dump({k:v for k,v in result.items() if k!='shadow_signals'}, f, indent=2, default=str, ensure_ascii=False)
print('Guardado: ma_slope_case3_ENGINE_v7_FINAL_20260726.json', flush=True)
